...
const vertexShaderHardwareSkinning =
  /*#__PURE__*/ vertexShaderHardwareSkinningSource.replace(
    /\$\{MAX_NODES}/g,
    String(MAX_NODES)
  );
const vertexShaderHDHardwareSkinningOld =
  /*#__PURE__*/ vertexShaderHDHardwareSkinningOldSource.replace(
    /\$\{MAX_NODES}/g,
    String(MAX_NODES)
  );
const vertexShaderHDHardwareSkinningNew =
  /*#__PURE__*/ vertexShaderHDHardwareSkinningNewSource.replace(
    /\$\{MAX_NODES}/g,
    String(MAX_NODES)
  );
const fragmentShaderHDNew = /*#__PURE__*/ fragmentShaderHDNewSource.replace(
  /\$\{MAX_ENV_MIP_LEVELS}/g,
  String(MAX_ENV_MIP_LEVELS.toFixed(1))
);
const sdShader = /*#__PURE__*/ sdShaderSource.replace(
  /\$\{MAX_NODES}/g,
  String(MAX_NODES)
);
const hdShader = /*#__PURE__*/ hdShaderSource
  .replace(/\$\{MAX_NODES}/g, String(MAX_NODES))
  .replace(/\$\{MAX_ENV_MIP_LEVELS}/g, String(MAX_ENV_MIP_LEVELS.toFixed(1)));
const depthShader = /*#__PURE__*/ depthShaderSource.replace(
  /\$\{MAX_NODES}/g,
  String(MAX_NODES)
);
...
const GPU_LAYER_PROPS: [string, GPUBlendState, GPUDepthStencilState][] = [
  [
    "none",
    {
      color: {
...
export class ModelRenderer {
  ...
  private envToCubemap: WebGLProgramObject<
    "aPos",
    "uPMatrix" | "uMVMatrix" | "uEquirectangularMap"
  >;
  ...
  private envSphere: WebGLProgramObject<
    "aPos",
    "uPMatrix" | "uMVMatrix" | "uEnvironmentMap"
  >;
  private convoluteDiffuseEnv: WebGLProgramObject<
    "aPos",
    "uPMatrix" | "uMVMatrix" | "uEnvironmentMap"
  >;
  ...
  private prefilterEnv: WebGLProgramObject<
    "aPos",
    "uPMatrix" | "uMVMatrix" | "uEnvironmentMap" | "uRoughness"
  >;
  ...
  constructor(model: Model) {
    ...
    for (const node of model.Nodes) {
      if (node) {
        if (!node.Parent && node.Parent !== 0) {
          this.rendererData.rootNode.childs.push(
            this.rendererData.nodes[node.ObjectId]
          );
        } else {
          this.rendererData.nodes[node.Parent].childs.push(
            this.rendererData.nodes[node.ObjectId]
          );
        }
      }
    ...
    for (let i = 0; i < model.GeosetAnims.length; ++i) {
      this.rendererData.geosetAnims[model.GeosetAnims[i].GeosetId] =
        model.GeosetAnims[i];
    }

    for (let i = 0; i < model.Materials.length; ++i) {
      this.rendererData.materialLayerTextureID[i] = new Array(
        model.Materials[i].Layers.length
      );
      this.rendererData.materialLayerNormalTextureID[i] = new Array(
        model.Materials[i].Layers.length
      );
      this.rendererData.materialLayerOrmTextureID[i] = new Array(
        model.Materials[i].Layers.length
      );
      this.rendererData.materialLayerReflectionTextureID[i] = new Array(
        model.Materials[i].Layers.length
      );
    }
    ...
    this.ribbonsController = new RibbonsController(
      this.interp,
      this.rendererData
    );
  }
    ...
    if (this.gl) {
      if (this.skeletonShaderProgram) {
        if (this.skeletonVertexShader) {
          this.gl.detachShader(
            this.skeletonShaderProgram,
            this.skeletonVertexShader
          );
      ...
      this.model.Materials.forEach((material) => {
        ...
        if (
          (material.Shader === "Shader_HD_DefaultUnit" &&
            material.Layers.length === 6 &&
            typeof material.Layers[5].TextureID === "number") ||
          (this.model.Version >= 1100 &&
            (layer = material.Layers.find(
              (it) => it.ShaderTypeId === 1 && it.ReflectionsTextureID
            )) &&
            typeof layer.ReflectionsTextureID === "number")
        ) {
          const id =
            this.model.Version >= 1100 && layer
              ? layer.ReflectionsTextureID
              : material.Layers[5].TextureID;
          this.rendererData.requiredEnvMaps[this.model.Textures[id].Image] =
            true;
        }
      });
    }
  ...
  public setTextureImage(path: string, img: HTMLImageElement): void {
    if (this.device) {
      const texture = (this.rendererData.gpuTextures[path] =
      ...
      const flags =
        this.model.Textures.find((it) => it.Image === path)?.Flags || 0;
  ...
  public setTextureCompressedImage(
    path: string,
    format: DDS_FORMAT,
    ...
    if (isWebGL2(this.gl)) {
      this.gl.texStorage2D(
        ...
        count,
        format,
        ddsInfo.images[0].shape.width,
        ddsInfo.images[0].shape.height
      );
  ...
  public setGPUTextureCompressedImage(
    path: string,
    ...
    for (let i = 0; i < count; ++i) {
      ...
      this.device.queue.writeTexture(
        ...
        {
          bytesPerRow:
            image.shape.width * (format === "bc1-rgba-unorm" ? 2 : 4),
        },
  ...
  public setSequence(index: number): void {
    ...
    this.rendererData.animationInfo =
      this.model.Sequences[this.rendererData.animation];
  ...
  public setFrame(frame: number): void {
    const index = this.model.Sequences.findIndex(
      (it) => it.Interval[0] <= frame && it.Interval[1] >= frame
    );
  ...
  public update(delta: number): void {
    ...
    for (
      let materialId = 0;
      materialId < this.rendererData.materialLayerTextureID.length;
      ++materialId
    ) {
      for (
        let layerId = 0;
        layerId < this.rendererData.materialLayerTextureID[materialId].length;
        ++layerId
      ) {
        ...
        const ReflectionsTextureID: AnimVector | number =
          layer.ReflectionsTextureID;

        if (typeof TextureID === "number") {
          this.rendererData.materialLayerTextureID[materialId][layerId] =
            TextureID;
        } else {
          this.rendererData.materialLayerTextureID[materialId][layerId] =
            this.interp.num(TextureID);
        }
        if (typeof NormalTextureID !== "undefined") {
          this.rendererData.materialLayerNormalTextureID[materialId][layerId] =
            typeof NormalTextureID === "number"
              ? NormalTextureID
              : this.interp.num(NormalTextureID);
        }
        if (typeof ORMTextureID !== "undefined") {
          this.rendererData.materialLayerOrmTextureID[materialId][layerId] =
            typeof ORMTextureID === "number"
              ? ORMTextureID
              : this.interp.num(ORMTextureID);
        }
        if (typeof ReflectionsTextureID !== "undefined") {
          this.rendererData.materialLayerReflectionTextureID[materialId][
            layerId
          ] =
            typeof ReflectionsTextureID === "number"
              ? ReflectionsTextureID
              : this.interp.num(ReflectionsTextureID);
        }
      }
    }
  }
  ...
  public render(
    ...
    if (this.device) {
      if (
        this.gpuMultisampleTexture.width !== this.canvas.width ||
        this.gpuMultisampleTexture.height !== this.canvas.height
      ) {
        this.gpuMultisampleTexture.destroy();
        this.initGPUMultisampleTexture();
      }
      ...
      if (
        this.gpuDepthTexture.width !== this.canvas.width ||
        this.gpuDepthTexture.height !== this.canvas.height
      ) {
        this.gpuDepthTexture.destroy();
        this.initGPUDepthTexture();
      }
      ...
      for (let j = 0; j < MAX_NODES; ++j) {
        if (this.rendererData.nodes[j]) {
          VSUniformsViews.nodesMatrices.set(
      ...
      for (let i = 0; i < this.model.Geosets.length; ++i) {
        ...
        if (
          geoset.LevelOfDetail !== undefined &&
          geoset.LevelOfDetail !== levelOfDetail
        ) {
          continue;
        }
        ...
        pass.setIndexBuffer(
          wireframe ? this.wireframeIndexGPUBuffer[i] : this.gpuIndexBuffer[i],
          "uint16"
        );
        ...
        if (this.isHD) {
          ...
          if (
            depthTextureTarget &&
            !FILTER_MODES_WITH_DEPTH_WRITE.has(baseLayer.FilterMode || 0)
          ) {
            continue;
          }
          const pipeline = depthTextureTarget
            ...
            : wireframe
            ? this.gpuWireframePipeline
            : this.getGPUPipeline(baseLayer);
          pass.setPipeline(pipeline);

          const textures = this.rendererData.materialLayerTextureID[materialID];
          const normalTextres =
            this.rendererData.materialLayerNormalTextureID[materialID];
          const ormTextres =
            this.rendererData.materialLayerOrmTextureID[materialID];
          const envTextres =
            this.rendererData.materialLayerReflectionTextureID[materialID];
          const diffuseTextureID = textures[0];
          const diffuseTexture = this.model.Textures[diffuseTextureID];
          const normalTextureID =
            baseLayer?.ShaderTypeId === 1 ? normalTextres[0] : textures[1];
          const normalTexture = this.model.Textures[normalTextureID];
          const ormTextureID =
            baseLayer?.ShaderTypeId === 1 ? ormTextres[0] : textures[2];
          const ormTexture = this.model.Textures[ormTextureID];
          const envTextureID =
            baseLayer?.ShaderTypeId === 1 ? envTextres[0] : textures[5];
          const envTexture = this.model.Textures[envTextureID];

          const envTextureImage = envTexture?.Image;
          const irradianceMap =
            this.rendererData.gpuIrradianceMap[envTextureImage];
          const prefilteredEnv =
            this.rendererData.gpuPrefilteredEnvMap[envTextureImage];
          ...
          if (!gpuFSUniformsBuffer) {
            gpuFSUniformsBuffer = this.gpuFSUniformsBuffers[materialID][0] =
          ...
          FSUniformsViews.discardAlphaLevel.set([
            baseLayer.FilterMode === FilterMode.Transparent ? 0.75 : 0,
          ]);
          ...
          const fsBindGroup = this.device.createBindGroup({
            ...
            entries: [
              ...
              },
              {
                binding: 2,
                resource: (
                  this.rendererData.gpuTextures[diffuseTexture.Image] ||
              ...
              },
              {
                binding: 4,
                resource: (
                  this.rendererData.gpuTextures[normalTexture.Image] ||
                  this.rendererData.gpuEmptyTexture
                ).createView(),
              },
              {
              ...
              },
              {
                binding: 6,
                resource: (
                  this.rendererData.gpuTextures[ormTexture.Image] ||
                  this.rendererData.gpuEmptyTexture
                ).createView(),
              },
              {
              ...
              },
              {
                binding: 8,
                resource: (
                  (shadowMapTexture as GPUTexture) ||
          ...
          for (let j = 0; j < material.Layers.length; ++j) {
            ...
            if (!gpuFSUniformsBuffer) {
              gpuFSUniformsBuffer = this.gpuFSUniformsBuffers[materialID][j] =
                this.device.createBuffer({
            ...
            FSUniformsViews.replaceableType.set([texture.ReplaceableId || 0]);
            FSUniformsViews.discardAlphaLevel.set([
              layer.FilterMode === FilterMode.Transparent ? 0.75 : 0,
            ]);
            FSUniformsViews.tVertexAnim.set(tVetexAnim.slice(0, 3));
            FSUniformsViews.tVertexAnim.set(tVetexAnim.slice(3, 6), 4);
            FSUniformsViews.tVertexAnim.set(tVetexAnim.slice(6, 9), 8);
            ...
            const fsBindGroup = this.device.createBindGroup({
              ...
              entries: [
                ...
                },
                {
                  binding: 2,
                  resource: (
                    this.rendererData.gpuTextures[texture.Image] ||
                    this.rendererData.gpuEmptyTexture
                  ).createView(),
                },
              ],
    ...
    this.gl.uniformMatrix4fv(
      this.shaderProgramLocations.pMatrixUniform,
      false,
      pMatrix
    );
    this.gl.uniformMatrix4fv(
      this.shaderProgramLocations.mvMatrixUniform,
      false,
      mvMatrix
    );
    this.gl.uniform1f(
      this.shaderProgramLocations.wireframeUniform,
      wireframe ? 1 : 0
    );
    ...
    this.gl.enableVertexAttribArray(
      this.shaderProgramLocations.normalsAttribute
    );
    this.gl.enableVertexAttribArray(
    ...
    if (this.isHD) {
      this.gl.enableVertexAttribArray(
        this.shaderProgramLocations.skinAttribute
      );
      this.gl.enableVertexAttribArray(
        this.shaderProgramLocations.weightAttribute
      );
      this.gl.enableVertexAttribArray(
        this.shaderProgramLocations.tangentAttribute
      );
    } else {
      if (!this.softwareSkinning) {
        this.gl.enableVertexAttribArray(
          this.shaderProgramLocations.groupAttribute
        );
      }
    }
    ...
    for (let i = 0; i < this.model.Geosets.length; ++i) {
      ...
      if (this.isHD) {
        this.gl.uniform3fv(
          this.shaderProgramLocations.lightPosUniform,
          this.rendererData.lightPos
        );
        this.gl.uniform3fv(
          this.shaderProgramLocations.lightColorUniform,
          this.rendererData.lightColor
        );
        // this.gl.uniform3fv(this.shaderProgramLocations.lightPosUniform, this.rendererData.cameraPos);
        this.gl.uniform3fv(
          this.shaderProgramLocations.cameraPosUniform,
          this.rendererData.cameraPos
        );

        if (shadowMapTexture && shadowMapMatrix) {
          this.gl.uniform3f(
            this.shaderProgramLocations.shadowParamsUniform,
            1,
            shadowBias ?? 1e-6,
            shadowSmoothingStep ?? 1 / 1024
          );
          ...
          this.gl.uniform1i(
            this.shaderProgramLocations.shadowMapSamplerUniform,
            3
          );
          this.gl.uniformMatrix4fv(
        ...
        const envTextureId =
          (this.model.Version >= 1100 &&
            material.Layers.find(
              (it) =>
                it.ShaderTypeId === 1 &&
                typeof it.ReflectionsTextureID === "number"
            )?.ReflectionsTextureID) ||
          material.Layers[5]?.TextureID;
        ...
        if (useEnvironmentMap && irradianceMap && prefilteredEnv) {
          ...
          this.gl.uniform1i(
            this.shaderProgramLocations.irradianceMapUniform,
            4
          );
          this.gl.activeTexture(this.gl.TEXTURE5);
          this.gl.bindTexture(this.gl.TEXTURE_CUBE_MAP, prefilteredEnv);
          this.gl.uniform1i(
            this.shaderProgramLocations.prefilteredEnvUniform,
            5
          );
        ...
        this.gl.bindBuffer(
          ...
          wireframe ? this.wireframeIndexBuffer[i] : this.indexBuffer[i]
        );
  ...
  private renderEnvironment(mvMatrix: mat4, pMatrix: mat4): void {
    ...
    for (const path in this.rendererData.envTextures) {
      ...
      this.gl.uniformMatrix4fv(
        this.envSphere.uniforms.uPMatrix,
        false,
        pMatrix
      );
      this.gl.uniformMatrix4fv(
        this.envSphere.uniforms.uMVMatrix,
        false,
        mvMatrix
      );
      ...
      this.gl.bindTexture(
        ...
        this.rendererData.envTextures[path]
      );
      ...
      this.gl.vertexAttribPointer(
        this.envSphere.attributes.aPos,
        3,
        this.gl.FLOAT,
        false,
        0,
  ...
  public renderSkeleton(
    ...
    const updateNode = (node: NodeWrapper) => {
      if (
        (node.node.Parent || node.node.Parent === 0) &&
        (!nodes || nodes.includes(node.node.Name))
      ) {
    ...
    this.gl.uniformMatrix4fv(
      this.skeletonShaderProgramLocations.pMatrixUniform,
      false,
      pMatrix
    );
    this.gl.uniformMatrix4fv(
      this.skeletonShaderProgramLocations.mvMatrixUniform,
      false,
      mvMatrix
    );

    this.gl.enableVertexAttribArray(
      this.skeletonShaderProgramLocations.vertexPositionAttribute
    );
    this.gl.enableVertexAttribArray(
      this.skeletonShaderProgramLocations.colorAttribute
    );
  ...
  private initSkeletonShaderProgram(): WebGLProgram {
    const vertex = (this.skeletonVertexShader = getShader(
      this.gl,
      skeletonVertexShader,
      this.gl.VERTEX_SHADER
    ));
    const fragment = (this.skeletonFragmentShader = getShader(
      this.gl,
    ...
    this.skeletonShaderProgramLocations.pMatrixUniform =
      this.gl.getUniformLocation(shaderProgram, "uPMatrix");
    this.skeletonShaderProgramLocations.mvMatrixUniform =
      this.gl.getUniformLocation(shaderProgram, "uMVMatrix");
      ...
      for (let j = 0; j < group.length; ++j) {
        vec3.add(
          ...
          vec3.transformMat4(
            tempVec3,
            tempPos,
            this.rendererData.nodes[group[j]].matrix
          )
        );
      }
  ...
  private setTextureParameters(flags: TextureFlags | 0, hasMipmaps: boolean) {
    ...
    this.gl.texParameteri(
      ...
      hasMipmaps ? this.gl.LINEAR_MIPMAP_NEAREST : this.gl.LINEAR
    );
  ...
  private processEnvMaps(path: string): void {
    ...
    mat4.perspective(pMatrix, Math.PI / 2, 1, 0.1, 10);
    ...
    if (this.device) {
      gpuCubemap = this.rendererData.gpuEnvTextures[path] =
        this.device.createTexture({
      ...
      this.gl.vertexAttribPointer(
        this.envToCubemap.attributes.aPos,
        3,
        this.gl.FLOAT,
        false,
        0,
      ...
      this.gl.uniformMatrix4fv(
        this.envToCubemap.uniforms.uPMatrix,
        false,
        pMatrix
      );
      this.gl.activeTexture(this.gl.TEXTURE0);
      ...
      for (let i = 0; i < 6; ++i) {
        ...
        mat4.lookAt(mvMatrix, eye, center[i], up[i]);
        this.gl.uniformMatrix4fv(
          this.envToCubemap.uniforms.uMVMatrix,
          false,
          mvMatrix
        );
    ...
    if (this.device) {
      gpuCubemap = this.rendererData.gpuIrradianceMap[path] =
        this.device.createTexture({
      ...
      const diffuseCubemap = (this.rendererData.irradianceMap[path] =
      ...
      this.gl.vertexAttribPointer(
        this.convoluteDiffuseEnv.attributes.aPos,
        3,
        this.gl.FLOAT,
        false,
        0,
      ...
      this.gl.uniformMatrix4fv(
        this.convoluteDiffuseEnv.uniforms.uPMatrix,
        false,
        pMatrix
      );
      this.gl.activeTexture(this.gl.TEXTURE0);
      ...
      for (let i = 0; i < 6; ++i) {
        ...
        mat4.lookAt(mvMatrix, eye, center[i], up[i]);
        this.gl.uniformMatrix4fv(
          this.convoluteDiffuseEnv.uniforms.uMVMatrix,
          false,
          mvMatrix
        );
    ...
    if (this.device) {
      const prefilterEnv = (this.rendererData.gpuPrefilteredEnvMap[path] =
        this.device.createTexture({
      ...
      const prefilterCubemap = (this.rendererData.prefilteredEnvMap[path] =
        this.gl.createTexture());
      this.gl.activeTexture(this.gl.TEXTURE1);
      ...
      for (let mip = 0; mip < MAX_ENV_MIP_LEVELS; ++mip) {
        for (let i = 0; i < 6; ++i) {
          const size = ENV_PREFILTER_SIZE * 0.5 ** mip;
      ...
      this.gl.vertexAttribPointer(
        this.prefilterEnv.attributes.aPos,
        3,
        this.gl.FLOAT,
        false,
        0,
      ...
      this.gl.uniformMatrix4fv(
        this.prefilterEnv.uniforms.uPMatrix,
        false,
        pMatrix
      );
      this.gl.activeTexture(this.gl.TEXTURE0);
      ...
      for (let mip = 0; mip < MAX_ENV_MIP_LEVELS; ++mip) {
        const mipWidth = ENV_PREFILTER_SIZE * 0.5 ** mip;
        const mipHeight = ENV_PREFILTER_SIZE * 0.5 ** mip;
        ...
        for (let i = 0; i < 6; ++i) {
          ...
          mat4.lookAt(mvMatrix, eye, center[i], up[i]);
          this.gl.uniformMatrix4fv(
            this.prefilterEnv.uniforms.uMVMatrix,
            false,
            mvMatrix
          );
  ...
  private destroyShaderProgramObject<A extends string, U extends string>(
    object: WebGLProgramObject<A, U>
  ): void {
    if (object.program) {
  ...
  private initShaders(): void {
    ...
    if (this.isHD) {
      vertexShaderSource = isWebGL2(this.gl)
        ? vertexShaderHDHardwareSkinningNew
        : vertexShaderHDHardwareSkinningOld;
    ...
    if (this.isHD) {
      fragmentShaderSource = isWebGL2(this.gl)
        ? fragmentShaderHDNew
        : fragmentShaderHDOld;
    } else {
    ...
    const vertex = (this.vertexShader = getShader(
      this.gl,
      vertexShaderSource,
      this.gl.VERTEX_SHADER
    ));
    const fragment = (this.fragmentShader = getShader(
      this.gl,
    ...
    if (this.isHD) {
      this.shaderProgramLocations.skinAttribute = this.gl.getAttribLocation(
        shaderProgram,
        "aSkin"
      );
      this.shaderProgramLocations.weightAttribute = this.gl.getAttribLocation(
        shaderProgram,
        "aBoneWeight"
      );
      this.shaderProgramLocations.tangentAttribute = this.gl.getAttribLocation(
        shaderProgram,
        "aTangent"
      );
    } else {
      if (!this.softwareSkinning) {
        this.shaderProgramLocations.groupAttribute = this.gl.getAttribLocation(
          shaderProgram,
          "aGroup"
        );
      }
    ...
    this.shaderProgramLocations.pMatrixUniform = this.gl.getUniformLocation(
      shaderProgram,
      "uPMatrix"
    );
    this.shaderProgramLocations.mvMatrixUniform = this.gl.getUniformLocation(
      shaderProgram,
      "uMVMatrix"
    );
    this.shaderProgramLocations.samplerUniform = this.gl.getUniformLocation(
      shaderProgram,
      "uSampler"
    );
    ...
    if (this.isHD) {
      this.shaderProgramLocations.normalSamplerUniform =
        this.gl.getUniformLocation(shaderProgram, "uNormalSampler");
      this.shaderProgramLocations.ormSamplerUniform =
        this.gl.getUniformLocation(shaderProgram, "uOrmSampler");
      this.shaderProgramLocations.lightPosUniform = this.gl.getUniformLocation(
        shaderProgram,
        "uLightPos"
      );
      this.shaderProgramLocations.lightColorUniform =
        this.gl.getUniformLocation(shaderProgram, "uLightColor");
      this.shaderProgramLocations.cameraPosUniform = this.gl.getUniformLocation(
        shaderProgram,
        "uCameraPos"
      );

      this.shaderProgramLocations.shadowParamsUniform =
        this.gl.getUniformLocation(shaderProgram, "uShadowParams");
      this.shaderProgramLocations.shadowMapSamplerUniform =
        this.gl.getUniformLocation(shaderProgram, "uShadowMapSampler");
      this.shaderProgramLocations.shadowMapLightMatrixUniform =
        this.gl.getUniformLocation(shaderProgram, "uShadowMapLightMatrix");
      ...
      this.shaderProgramLocations.irradianceMapUniform =
        this.gl.getUniformLocation(shaderProgram, "uIrradianceMap");
      this.shaderProgramLocations.prefilteredEnvUniform =
        this.gl.getUniformLocation(shaderProgram, "uPrefilteredEnv");
      this.shaderProgramLocations.brdfLUTUniform = this.gl.getUniformLocation(
        shaderProgram,
        "uBRDFLUT"
      );
    } else {
    ...
    this.shaderProgramLocations.tVertexAnimUniform = this.gl.getUniformLocation(
      shaderProgram,
      "uTVertexAnim"
    );
    ...
    if (this.isHD && isWebGL2(this.gl)) {
      this.envToCubemap = this.initShaderProgram(
        envToCubemapVertexShader,
        envToCubemapFragmentShader,
        {
          aPos: "aPos",
        },
        {
      ...
      this.envSphere = this.initShaderProgram(
        envVertexShader,
        envFragmentShader,
        {
          aPos: "aPos",
      ...
      this.convoluteDiffuseEnv = this.initShaderProgram(
        convoluteEnvDiffuseVertexShader,
        convoluteEnvDiffuseFragmentShader,
        {
          aPos: "aPos",
        },
        {
      ...
      this.prefilterEnv = this.initShaderProgram(
        prefilterEnvVertexShader,
        prefilterEnvFragmentShader,
        {
          aPos: "aPos",
        },
        {
      ...
      this.integrateBRDF = this.initShaderProgram(
        integrateBRDFVertexShader,
        integrateBRDFFragmentShader,
        {
          aPos: "aPos",
        },
        {}
  ...
  private initGPUShaders(): void {
    ...
    for (let i = 0; i < this.model.Textures.length; ++i) {
      ...
      const addressModeU: GPUAddressMode =
        flags & TextureFlags.WrapWidth ? "repeat" : "clamp-to-edge";
      const addressModeV: GPUAddressMode =
        flags & TextureFlags.WrapHeight ? "repeat" : "clamp-to-edge";
    ...
    if (this.isHD) {
      ...
      this.envToCubemapVSBindGroupLayout =
        this.envToCubemapPiepeline.getBindGroupLayout(0);
      ...
      this.envToCubemapFSBindGroupLayout =
        this.envToCubemapPiepeline.getBindGroupLayout(1);
      ...
      this.convoluteDiffuseEnvVSBindGroupLayout =
        this.convoluteDiffuseEnvPiepeline.getBindGroupLayout(0);
      this.convoluteDiffuseEnvFSBindGroupLayout =
        this.convoluteDiffuseEnvPiepeline.getBindGroupLayout(1);
      ...
      this.prefilterEnvVSBindGroupLayout =
        this.prefilterEnvPiepeline.getBindGroupLayout(0);
      this.prefilterEnvFSBindGroupLayout =
        this.prefilterEnvPiepeline.getBindGroupLayout(1);
  ...
  private createWireframeBuffer(index: number): void {
    ...
    this.gl.bindBuffer(
      this.gl.ELEMENT_ARRAY_BUFFER,
      this.wireframeIndexBuffer[index]
    );
    this.gl.bufferData(
      this.gl.ELEMENT_ARRAY_BUFFER,
      lines,
  ...
  private createWireframeGPUBuffer(index: number): void {
    ...
    new Uint16Array(
      this.wireframeIndexGPUBuffer[index].getMappedRange(
        0,
        this.wireframeIndexGPUBuffer[index].size
      )
    ).set(lines);
  ...
  private getGPUPipeline(layer: Layer): GPURenderPipeline {
    ...
    if (!this.gpuPipelines[key]) {
      this.gpuPipelines[key] = this.createGPUPipelineByLayer(
        filterMode,
        twoSided
      );
    }
  ...
  private initGPUPipeline(): void {
    ...
    this.gpuWireframePipeline = this.createGPUPipeline(
      "wireframe",
      {
        color: {
          operation: "add",
    ...
    if (this.isHD) {
      this.gpuShadowPipeline = this.createGPUPipeline(
        "shadow",
        undefined,
        {
          depthWriteEnabled: true,
  ...
  private initGPUBuffers(): void {
    for (let i = 0; i < this.model.Geosets.length; ++i) {
      ...
      new Float32Array(
        this.gpuTexCoordBuffer[i].getMappedRange(
          0,
          this.gpuTexCoordBuffer[i].size
        )
      ...
      if (this.isHD) {
        ...
        new Uint8Array(
          this.gpuSkinWeightBuffer[i].getMappedRange(
            0,
            this.gpuSkinWeightBuffer[i].size
          )
        ...
        new Float32Array(
          this.gpuTangentBuffer[i].getMappedRange(
            0,
            this.gpuTangentBuffer[i].size
          )
        ).set(geoset.Tangents);
      ...
      new Uint16Array(this.gpuIndexBuffer[i].getMappedRange(0, size)).set(
        geoset.Faces
      );
  ...
  private initGPUEmptyTexture(): void {
    const texture = (this.rendererData.gpuEmptyTexture =
      this.device.createTexture({
        label: "empty texture",
        size: [1, 1],
        format: "rgba8unorm",
  ...
  private initBRDFLUT(): void {
    ...
    this.gl.vertexAttribPointer(
      this.integrateBRDF.attributes.aPos,
      2,
      this.gl.FLOAT,
      false,
      0,
  ...
  private updateGlobalSequences(delta: number): void {
    for (let i = 0; i < this.rendererData.globalSequencesFrames.length; ++i) {
      ...
      if (
        this.rendererData.globalSequencesFrames[i] >
        this.model.GlobalSequences[i]
      ) {
        this.rendererData.globalSequencesFrames[i] = 0;
      }
    }
  ...
  private updateNode(node: NodeWrapper): void {
    ...
    if (node.node.Parent || node.node.Parent === 0) {
      mat4.mul(
        node.matrix,
        this.rendererData.nodes[node.node.Parent].matrix,
        node.matrix
      );
    }
  ...
  private getTexCoordMatrix(layer: Layer): mat3 {
    if (typeof layer.TVertexAnimId === "number") {
      const anim: TVertexAnim =
        this.rendererData.model.TextureAnims[layer.TVertexAnimId];
  ...
  private setLayerProps(layer: Layer, textureID: number): void {
    ...
    if (texture.Image) {
      ...
      this.gl.bindTexture(
        this.gl.TEXTURE_2D,
        this.rendererData.textures[texture.Image]
      );
    ...
    this.gl.uniformMatrix3fv(
      this.shaderProgramLocations.tVertexAnimUniform,
      false,
      this.getTexCoordMatrix(layer)
    );
  }
  ...
  private setLayerPropsHD(materialID: number, layers: Layer[]): void {
    ...
    this.gl.bindTexture(
      this.gl.TEXTURE_2D,
      this.rendererData.textures[diffuseTexture.Image]
    );
    this.gl.uniform1i(this.shaderProgramLocations.samplerUniform, 0);
    ...
    if (typeof baseLayer.TVertexAnimId === "number") {
      const anim: TVertexAnim =
        this.rendererData.model.TextureAnims[baseLayer.TVertexAnimId];
      const translationRes = this.interp.vec3(translation, anim.Translation);
      const rotationRes = this.interp.quat(rotation, anim.Rotation);
      const scalingRes = this.interp.vec3(scaling, anim.Scaling);
      mat4.fromRotationTranslationScale(
    ...
    this.gl.bindTexture(
      this.gl.TEXTURE_2D,
      this.rendererData.textures[normalTexture.Image]
    );
    ...
    this.gl.bindTexture(
      this.gl.TEXTURE_2D,
      this.rendererData.textures[ormTexture.Image]
    );
